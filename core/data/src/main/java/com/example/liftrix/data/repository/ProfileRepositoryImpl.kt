    ...
    override suspend fun deleteProfile(userId: String): Result<Unit> {
        return try {
            if (OfflineArchitectureFlags.FIX_PROFILE_REPOSITORY) {
                ...
                syncScheduler.triggerEntitySync(userId, "profile")
            } else {
                userProfileDao.deleteProfileForUser(userId)
                legacyDeleteProfile(userId)
            }

            syncScheduler.cancelSyncForUser(userId)
        ...
        return try {
            ...
            if (profile != null && !profile.isSynced) {
                ...
                if (queueResult.isSuccess) {
                    ...
                    val syncResult = syncScheduler.triggerEntitySync(userId, "profile")
                    ...
                    val publicSyncResult = syncScheduler.triggerEntitySync(userId, "user_public")
