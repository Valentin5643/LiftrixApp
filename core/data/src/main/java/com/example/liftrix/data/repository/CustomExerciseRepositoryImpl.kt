    ...
    override suspend fun createCustomExercise(
        ...
        try {
            ...
            val mainImageUrl = mainImage?.let { uri ->
                val uploadResult = mediaUploadService.uploadImage(uri as Uri, "exercises/$userId")
                uploadResult.fold(
                    onSuccess = { it },
            ...
            for (imageUri in additionalImages) {
                val uploadResult = mediaUploadService.uploadImage(imageUri as Uri, "exercises/$userId")
                uploadResult.fold(
