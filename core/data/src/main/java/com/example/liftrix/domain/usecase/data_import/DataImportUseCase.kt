 ...
 * - Queries: validateFile(), getImportProgress()
 * - Commands: import(), rollbackImport()
 *
...
class DataImportUseCaseImpl @Inject constructor(
    ...
    /**
     * Imports workouts from a file with validation and conflict resolution.
     *
     * **Replaces**: ImportWorkoutsUseCase.importWorkouts()
     *
     * This operation:
     * 1. Creates import tracking record
     * 2. Parses and validates file contents
     * 3. Processes workouts with conflict resolution
     * 4. Updates import tracking with results
     * 5. Returns import result with counts and errors
     *
     * @param userId User ID for scoped import
     * @param uri URI of the import file
     * @param inputStream Input stream for reading file contents
    ...
    override suspend fun import(
        ...
        errorMapper = { throwable ->
            LiftrixError.BusinessLogicError(
                ...
                errorMessage = "Failed to import workout data: ${throwable.message}",
                analyticsContext = mapOf(
                    "user_id" to userId,
                    "uri" to uri.toString(),
        ...
        try {
            ...
            // Validate data
            val validationResult = validate(workouts, userId)
            val validation = validationResult.getOrThrow()
    ...
    /**
     * Rolls back a previously completed import.
     *
     * **Replaces**: ImportWorkoutsUseCase.rollbackImport()
     *
     * Removes imported workouts and updates import tracking.
     * Operation is only available for recently completed imports (within rollback window).
     *
     * @param importId ID of the import to rollback
     * @param userId User ID for scoped rollback
    ...
    suspend fun rollbackImport(importId: String, userId: String): LiftrixResult<Unit> = liftrixCatching(
        errorMapper = { throwable ->
            LiftrixError.BusinessLogicError(
                ...
                errorMessage = "Failed to rollback import: ${throwable.message}",
                analyticsContext = mapOf(
        ...
        require(userId.isNotBlank()) { "User ID cannot be blank" }
        require(importId.isNotBlank()) { "Import ID cannot be blank" }
