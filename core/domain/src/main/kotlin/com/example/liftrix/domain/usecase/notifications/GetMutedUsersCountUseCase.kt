...
class GetMutedUsersCountUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): Flow<LiftrixResult<Int>> {
        return try {
            ...
            Timber.d("Retrieving muted users count for user: ${userId.value}")

            // Get muted users count from repository as reactive stream
            notificationRepository.getMutedUsersCount(userId.value)
                .map { count ->
                    Timber.d("Muted users count for user ${userId.value}: $count")
    ...
    suspend fun getMutedUsersCountForUser(userId: String): Flow<LiftrixResult<Int>> {
        return try {
            ...
            Timber.d("Retrieving muted users count for specific user: $userId")
    ...
    suspend fun getCurrentCount(): LiftrixResult<Int> = liftrixCatching(
        ...
        Timber.d("Getting current muted users count for user: ${userId.value}")

        // Get the first emission from the reactive stream
        val count = try {
            notificationRepository.getMutedUsersCount(userId.value).first()
        } catch (e: Exception) {
            Timber.w(e, "Error getting current muted count for user: ${userId.value}, returning 0")
            0 // Graceful fallback to zero
        }

        Timber.d("Current muted users count for user ${userId.value}: $count")
        count
    }
    ...
    suspend fun hasAnyMutedUsers(): LiftrixResult<Boolean> = liftrixCatching(
        ...
        countResult.fold(
            ...
            onFailure = { error ->
                Timber.w("Failed to get muted count, assuming no muted users: $error")
    ...
    suspend fun getMutedUsersList(): Flow<LiftrixResult<List<String>>> {
        return try {
            ...
            Timber.d("Retrieving muted users list for user: ${userId.value}")

            notificationRepository.getMutedUsers(userId.value)
                .map { mutedUsers ->
                    Timber.d("Retrieved ${mutedUsers.size} muted users for user: ${userId.value}")
