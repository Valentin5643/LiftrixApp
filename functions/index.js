 ...
 */
exports.updateUserCustomClaims = onDocumentWritten('subscriptions/{subscriptionId}', async (event) => {
  ...
  try {
    ...
    if (!subscriptionData) {
      logger.warn(`No subscription data found for subscription: ${subscriptionId}`);
      return null;
    }
    ...
    if (subscriptionData.claims_updated) {
      logger.info(`Claims already updated for subscription: ${subscriptionId}`);
      return null;
    }

    // Calculate custom claims based on subscription
    const customClaims = await calculateCustomClaims(userId, subscriptionData);
    ...
    logger.info(`Custom claims updated for user: ${userId}, subscription: ${subscriptionId}`, {
      userId,
      subscriptionId,
    ...
    logger.error(`Error updating custom claims for subscription: ${subscriptionId}`, error);
    throw error;
  }
});
 ...
 */
exports.cleanupExpiredSubscriptions = require('firebase-functions').pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      logger.info(`Cleaned up ${expiredSubscriptions.size} expired subscriptions`);
      return null;
    } catch (error) {
 ...
 */
exports.updateUserClaimsManually = require('firebase-functions').https.onCall(async (data, context) => {
  // Verify the request is from an authenticated admin user
  if (!context.auth || !context.auth.token.admin) {
    throw new require('firebase-functions').https.HttpsError(
      'permission-denied',
      'Only admin users can manually update claims'
    );
  }
  ...
  try {
    ...
    logger.error(`Error in manual claims update for user: ${userId}`, error);
    throw new require('firebase-functions').https.HttpsError(
      'internal',
