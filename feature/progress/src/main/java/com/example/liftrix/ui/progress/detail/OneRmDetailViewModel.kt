...
import com.example.liftrix.domain.progress.OneRmProgressionData as UseCaseOneRmProgressionData
...
class OneRmDetailViewModel @Inject constructor(
    ...
    private val exerciseCatalogPort: ProgressExerciseCatalogPort,
    private val analyticsExportUseCase: ProgressDetailAnalyticsGateway
     ...
     */
    private val _availableExercises = MutableStateFlow<List<ProgressExerciseOption>>(emptyList())
            ...
            viewModelScope.launch {
                try {
                    val exportRequest = ExportOneRmDataRequest(
                        progressionPoints = currentState.data.progressionPoints.map { point ->
                            com.example.liftrix.domain.progress.OneRmDataPoint(
                                date = point.date,
                                exerciseId = point.exerciseId.orEmpty(),
                                exerciseName = point.exerciseName.orEmpty(),
                                actualOneRm = point.actualOneRm,
                                estimatedOneRm = point.estimatedOneRm,
                                weight = point.weight ?: point.bestOneRm,
                                reps = point.reps ?: 1,
                    ...
                    val result = analyticsQueryUseCase.exportOneRm(exportRequest)
                    result.fold(
                        onSuccess = { file ->
    ...
    private fun loadAvailableExercises() {
        viewModelScope.launch {
            try {
                val result = exerciseCatalogPort()
                result.fold(
                    onSuccess = { exercises ->
     ...
     */
    private fun createMockExercises(): List<ProgressExerciseOption> {
        return listOf(
