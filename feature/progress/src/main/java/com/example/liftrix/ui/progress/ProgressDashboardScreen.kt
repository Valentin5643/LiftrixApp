...
import com.example.liftrix.ui.progress.components.ProgressPrimaryActionButton
import com.example.liftrix.ui.progress.components.ProgressSecondaryActionButton
                ...
                if (featuresState.exportEnabled) {
                    ...
                    if (showExportBottomSheet) {
                        AlertDialog(
                            ...
                            text = { Text("Choose an export format for the current progress dashboard.") },
                            confirmButton = {
                                TextButton(
                                    onClick = {
            ...
            if (calorieState.calorieSummary is AsyncData.Success) {
                val summaryData = calorieState.calorieSummary.data
                    ...
                    if (weeklyCalories.isNotEmpty()) {
                        ProgressWeeklyCalorieTrendCard(
                            weeklyCalories = weeklyCalories,
                            averageCalories = summaryData.averageDailyCalories * 7,
...
private fun CalorieInsightsSummary(
    calorieSummary: ProgressCalorieSummary,
    modifier: Modifier = Modifier
) {
    Card( // Specialized calorie insights card
        ...
        Column(
            ...
            Row(
                ...
                InsightMetric(
                    label = "This Week",
                    value = "${calorieSummary.currentWeekCalories} cal",
                    icon = Icons.Default.Timeline
                )
                
                InsightMetric(
                    label = "Highest Day",
                    value = "${calorieSummary.highestDailyCalories} cal",
                    icon = Icons.Default.ShowChart
                )
                
                InsightMetric(
                    label = "Workouts",
                    value = "${calorieSummary.totalWorkouts}",
 ...
 */
private fun calculateCalorieTrend(calorieSummary: ProgressCalorieSummary): TrendDirection {
    return when {
        calorieSummary.weeklyTrend > 0f -> TrendDirection.UP
        calorieSummary.weeklyTrend == 0f -> TrendDirection.STABLE
        else -> TrendDirection.DOWN
    }
}

private fun getWeeklyCaloriesList(calorieSummary: ProgressCalorieSummary): List<Int> {
    return listOf(calorieSummary.previousWeekCalories, calorieSummary.currentWeekCalories)
        .filter { it > 0 }
}
