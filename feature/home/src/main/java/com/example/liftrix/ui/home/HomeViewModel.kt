...
class HomeViewModel @Inject constructor(
    private val homeWorkoutPort: HomeWorkoutPort,
    private val homeAuthPort: HomeAuthPort,
    private val authQueryUseCase: AuthQueryUseCase,
    private val analyticsService: HomeAnalyticsPort,
    private val socialRepository: HomeSocialPort,
    ...
    fun loadHomeData() {
        viewModelScope.launch {
            ...
            val result = homeWorkoutPort.getRecentWorkouts(userId, RECENT_WORKOUTS_LIMIT).first()
    ...
    fun loadFeedWorkouts() {
        ...
        feedObservationJob = viewModelScope.launch {
            try {
                ...
                homeWorkoutPort.getRecentActivityFeed(
                    userId = userId,
            ...
            homeAuthPort.currentUserId
                ...
                .collect { userId ->
                    if (userId != null) {
                        ...
                        if (currentObservedUserId != userId) {
    ...
    private fun observeWorkoutDataReactively(userId: String) {
        ...
        workoutObservationJob = viewModelScope.launch {
            ...
            homeWorkoutPort.getRecentWorkouts(userId, RECENT_WORKOUTS_LIMIT)
                .catch { throwable ->
                    // Properly handle cancellation vs actual errors
                    if (throwable !is kotlinx.coroutines.CancellationException) {
     ...
     */
    private fun trackWorkoutOpened(workout: HomeWorkout) {
        viewModelScope.launch {
            try {
                analyticsService.logEvent(
                    "home_workout_opened",
                    mapOf(
                        ...
                        "exercise_count" to workout.exerciseCount
                    )
                )
            } catch (exception: Exception) {
     ...
     */
    private fun trackFeedWorkoutOpened(feedWorkout: HomeFeedWorkout) {
        viewModelScope.launch {
            try {
                analyticsService.logEvent(
                    "home_feed_workout_opened",
                    mapOf(
                        "workout_id" to feedWorkout.workout.id,
                        "workout_name" to feedWorkout.workout.name,
                        "is_personal" to feedWorkout.isPersonal,
                        "user_id" to (feedWorkout.user?.uid ?: ""),
                        "exercise_count" to feedWorkout.workout.exerciseCount
                    )
                )
            } catch (exception: Exception) {
     ...
     */
    private val workoutStatsFlow: StateFlow<HomeWorkoutStats> = currentUserIdFlow
        .filterNotNull()
        .flatMapLatest { userId ->
            flow {
                try {
                    val result = homeWorkoutPort.getWorkoutStats(userId)
                    emit(result.getOrElse { HomeWorkoutStats.EMPTY })
                } catch (e: Exception) {
                    Timber.e(e, "Failed to load workout stats")
                    emit(HomeWorkoutStats.EMPTY)
                }
            }
        }
        ...
        .stateIn(
            ...
            initialValue = HomeWorkoutStats.EMPTY
        )
