...
class MediaProcessingServiceImpl @Inject constructor(
    ...
    override suspend fun processPhoto(
    ...
    ): LiftrixResult<ProcessedMedia> = liftrixCatching(
        errorMapper = { throwable ->
            LiftrixError.BusinessLogicError(
                code = "PHOTO_PROCESSING_FAILED",
                errorMessage = "Failed to process photo: ${throwable.message}",
                analyticsContext = mapOf(
                    "operation" to "PROCESS_PHOTO",
                    "uri" to uri.toString()
                )
            )
        }
        ...
        withContext(Dispatchers.IO) {
            ...
            if (!validation.isValid) {
                throw IllegalArgumentException("Invalid photo: ${validation.errors.joinToString()}")
            }
            
            // Load and process bitmap
            val inputStream = context.contentResolver.openInputStream(uri)
                ?: throw IllegalArgumentException("Cannot open photo URI")
    ...
    override suspend fun processVideo(
        ...
        errorMapper = { throwable ->
            LiftrixError.BusinessLogicError(
                ...
                errorMessage = "Failed to process video: ${throwable.message}",
                analyticsContext = mapOf(
                    "operation" to "PROCESS_VIDEO",
                    "uri" to uri.toString()
                )
            )
        }
        ...
        withContext(Dispatchers.IO) {
            ...
            if (!validation.isValid) {
                throw IllegalArgumentException("Invalid video: ${validation.errors.joinToString()}")
            }
    ...
    override suspend fun generateBlurhash(imageBytes: ByteArray): LiftrixResult<String> = 
        liftrixCatching(
            errorMapper = { throwable ->
                LiftrixError.BusinessLogicError(
                    code = "BLURHASH_GENERATION_FAILED",
                    errorMessage = "Failed to generate blurhash: ${throwable.message}",
                    analyticsContext = mapOf("operation" to "GENERATE_BLURHASH")
                )
            }
        ) {
    ...
    override suspend fun validateMedia(
        uri: Uri,
        mediaType: MediaType
    ): LiftrixResult<MediaValidationResult> = liftrixCatching(
        errorMapper = { throwable ->
            LiftrixError.ValidationError(
                field = "media_uri",
                violations = listOf("Media validation failed: ${throwable.message}"),
                analyticsContext = mapOf(
                    "operation" to "VALIDATE_MEDIA",
        ...
        if (inputStream == null) {
            return@liftrixCatching MediaValidationResult(
                isValid = false,
                fileSize = 0,
                format = "unknown",
                errors = errors
    ...
    override suspend fun estimateProcessing(
        uri: Uri,
        mediaType: MediaType
    ): LiftrixResult<ProcessingEstimate> = liftrixCatching(
        errorMapper = { throwable ->
            LiftrixError.BusinessLogicError(
                code = "PROCESSING_ESTIMATION_FAILED",
                errorMessage = "Failed to estimate processing: ${throwable.message}",
                analyticsContext = mapOf("operation" to "ESTIMATE_PROCESSING")
            )
        }
    ) {
    ...
    private fun estimatePhotoProcessingTime(fileSize: Long): Long {
        ...
        return (fileSize / 1024 / 1024 * 100).coerceAtLeast(500)
    }
    
    private fun estimateVideoProcessingTime(fileSize: Long, duration: Int?): Long {
        // Rough estimate: 500ms per MB + 100ms per second
        val sizeTime = fileSize / 1024 / 1024 * 500
        val durationTime = (duration ?: 10) * 100
        return (sizeTime + durationTime).coerceAtLeast(1000)
    }
}