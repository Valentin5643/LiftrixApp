...
class DashboardViewModel @Inject constructor(
    ...
    private suspend fun loadDashboard(forceRefresh: Boolean, showLoading: Boolean) {
        ...
        try {
            ...
            getDashboardConfigurationUseCase.execute(
                GetDashboardConfigurationUseCase.GetConfigurationParams(
                    userId = user.uid
                )
            ).collect { configResult ->
                configResult.fold(
                onSuccess = { configuration ->
                    // Update state with configuration - CRITICAL BUG FIX: Use WidgetResolver for proper widget resolution
    ...
    private suspend fun loadVisibleWidgets(widgetIds: List<String>, forceRefresh: Boolean) {
        ...
        widgetIds.forEach { widgetId ->
            ...
            try {
                ...
                // Load widget data
                val widgetResult = getWidgetDataUseCase.getWidgetData(user.uid, widget)
    ...
    private suspend fun refreshWidget(widgetId: String, forceRefresh: Boolean) {
        ...
        try {
            ...
            val result = if (forceRefresh) {
                getWidgetDataUseCase.refreshWidgetData(user.uid, widget)
            } else {
                getWidgetDataUseCase.getWidgetData(user.uid, widget)
            }
    ...
    private suspend fun toggleWidgetVisibility(widgetId: String, visible: Boolean?, shouldPersist: Boolean) {
        ...
        // CRITICAL BUG FIX: Use WidgetResolver for proper widget resolution  
        val resolvedWidgets = widgetResolver.resolveWidgetsFromPreferences(
        ...
        if (shouldPersist) {
            ...
            saveResult.fold(
                ...
                onFailure = { throwable ->
                    // CRITICAL BUG FIX: Revert using WidgetResolver for proper widget resolution
